...
from trcustoms.awards.specs import AwardSpec, magic_stones
...
def test_magic_stones_award_spec_success(
    spec: AwardSpec,
) -> None:
    user = UserFactory()
    tag1 = TagFactory()
...
@pytest.mark.django_db
def test_magic_stones_award_spec_not_enough_tags(
    spec: AwardSpec,
) -> None:
    user = UserFactory()
    tag1 = TagFactory()
...
@pytest.mark.django_db
def test_magic_stones_award_spec_too_far_apart(
    spec: AwardSpec,
) -> None:
    user = UserFactory()
    tag1 = TagFactory()
